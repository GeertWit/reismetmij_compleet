class Inboxes::DiscussionsController < ApplicationController
  ...
  def destroy
    ...
    flash[:notice] = t("discussions.removed")
    begin
      redirect_to :back
  ...
  def create
    ...
    if @discussion.save
      redirect_to discussion_url(@discussion), :notice => t("discussions.started")
    else
      render :action => "new"
    end
  end
  ...
  def load_and_check_discussion_recipient
    ...
    if @discussion.recipient_ids && @discussion.recipient_ids.size == 1
      ...
      if discussion
        ...
        redirect_to discussion_url(discussion), :notice => t("discussions.already_exists", :user => user[Inboxes::config.user_name])
      end
    end
  end
end
